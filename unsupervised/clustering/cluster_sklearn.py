...
def hierarchical(dataset, n_clusters, affinity='euclidean', linkage='ward'):
    return cluster.AgglomerativeClustering(n_clusters=4, affinity='euclidean',
        linkage='ward').fit_predict(dataset)

def hierarchical_connected(dataset, n_clusters, n_neighbors=5, include_self=False, affinity='euclidean', linkage='complete'):
...
def dirichlet(dataset, n_clusters, max_iter=100, covariance_type='full', weight_concentration_prior_type='dirichlet_process'):
    #covariance_type : {‘full’, ‘tied’, ‘diag’, ‘spherical’}, defaults to 'full'
    #init_params : {‘kmeans’, ‘random’}, defaults to ‘kmeans’.
    #weight_concentration_prior_type: 
    #   #'dirichlet_process' (using the Stick-breaking representation),
    #   #'dirichlet_distribution' (can favor more uniform weights).
    return mixture.BayesianGaussianMixture(n_components=n_clusters, covariance_type=covariance_type, weight_concentration_prior_type=weight_concentration_prior_type, max_iter=max_iter).fit_predict(dataset)
