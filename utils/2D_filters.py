...
def fl_anisotropic(data, c, its):
    for t in range(its):
        ...
        claplacian = iterate_matrix_by_const(laplacian, c**2, np.divide, as_array=False)
        claplacian = iterate_matrix_by_const(claplacian, -1, np.prod)
        data = iterate_matrix_func(claplacian, np.exp)
    return data
...
def example():
    ...
    print("5x5 mean")
    data = normalize(make_convolution_with_func(data, kernel5x5, np.mean))
    ...
    data = normalize(make_convolution_with_func(data, kernel5x5, np.max))
    #data = normalize(fl_anisotropic(data, 0.5, 4))
    print("data len:", len(data), ", y [0]:", len(data[0]))
    print("data mean: ", np.mean(data))
    print("data std: ", np.std(data))
    ...
    print("3x3 sum")
    data = np.copy(orig_data)
    data = normalize(make_convolution_with_func(data, kernel3x3, np.sum))
    #data = normalize(fl_anisotropic(data, 0.5, 4))
    print("data len:", len(data), ", y [0]:", len(data[0]))
    print("data mean: ", np.mean(data))
    print("data std: ", np.std(data))
    ...
    print("3x3 median")
    data = np.copy(orig_data)
    data = normalize(make_convolution_with_func(data, kernel3x3, np.median))
    #data = normalize(fl_anisotropic(data, 0.5, 4))
    print("data len:", len(data), ", y [0]:", len(data[0]))
    print("data mean: ", np.mean(data))
    print("data std: ", np.std(data))
