...
from mlexperiments.utils.change_types import list_to_str


def all_combinations_of_features(inputs: list, outputs: list,
                                 classification_algorithm: Callable[[list, list], tuple[Any, np.array]]):
    i_model = 0
    # max_i = 15
    best_acc = 0
    with open('output.txt', 'w') as output_file:
        ...
        for number_of_features in range(len(inputs[0])):
            ...
            for combination in list(combinations):
                ...
                output_file.write(str(number_of_features) + "," + list_to_str(combination, '+') + "," +
